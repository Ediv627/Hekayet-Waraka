...
// Utility to print / download an order invoice
...
function buildInvoiceBody(order: PrintOrder, adminMessage: string): string {
  const isHalf =
...
export function printOrderInvoice(order: PrintOrder, adminMessage: string) {
  const shortId = order.id.slice(0, 8).toUpperCase();
  const bodyHtml = buildInvoiceBody(order, adminMessage);
...
export async function downloadOrderInvoicePDF(order: PrintOrder, adminMessage: string) {
  const shortId = order.id.slice(0, 8).toUpperCase();
  const bodyHtml = buildInvoiceBody(order, adminMessage);
...
<body>${bodyHtml}</body>
  ...
  // Create hidden iframe to render invoice for capture
