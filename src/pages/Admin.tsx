...
const productSchema = z.object({
  ...
  shortDescription: z.string().max(100, 'الوصف القصير يجب ألا يتجاوز 100 حرف').optional(),
...
const Admin = () => {
  ...
  const [newArrivalsCount, setNewArrivalsCount] = useState('3');
  const [featuredProductEnabled, setFeaturedProductEnabled] = useState(false);
  ...
  useEffect(() => {
    const fetchStoreSettings = async () => {
      const { data, error } = await supabase
        .from('store_settings')
        .select('key, value')
        .in('key', ['store_email', 'store_phone', 'whatsapp_number', 'facebook_url', 'instagram_url', 'tiktok_url', 'vodafone_cash_number', 'free_delivery_threshold', 'branch_pickup_enabled', 'hero_image_url', 'branch_address', 'new_arrivals_count', 'featured_product_enabled', 'featured_product_id']);
      ...
      if (!error && data) {
        data.forEach((setting) => {
          ...
          else if (setting.key === 'new_arrivals_count') setNewArrivalsCount(setting.value || '3');
          else if (setting.key === 'featured_product_enabled') setFeaturedProductEnabled(setting.value === 'true');
          else if (setting.key === 'featured_product_id') setFeaturedProductId(setting.value || '');
        });
      }
    };
  ...
  const openEditDialog = (product: Product) => {
    ...
    setStockCount(
      product.stockCount === null || product.stockCount === undefined ? '' : String(product.stockCount)
    );
  ...
  const onSubmit = async (data: ProductFormData) => {
    ...
    try {
      ...
      const parsedStock = stockCount.trim() === '' ? null : parseInt(stockCount, 10);
      ...
      // Sync variants: delete all existing for product, then re-insert current list
      if (productId) {
        await supabase.from('product_variants').delete().eq('product_id', productId);
        if (cleanedVariants.length > 0) {
          const insertRows = cleanedVariants.map((v, idx) => ({
          ...
          if (variantError) {
            ...
            toast.error('تم حفظ المنتج لكن حدث خطأ في حفظ المقاسات');
          }
        }
      }
  ...
  const handleSaveSettings = async () => {
    ...
    let arrivalsValue = parseInt(newArrivalsCount, 10);
    if (isNaN(arrivalsValue) || arrivalsValue < 1) arrivalsValue = 1;
    if (arrivalsValue > 20) arrivalsValue = 20;
    ...
    try {
      const { error } = await supabase
        ...
        .upsert([
          ...
          { key: 'new_arrivals_count', value: arrivalsValue.toString() },
          { key: 'featured_product_enabled', value: featuredProductEnabled.toString() },
          { key: 'featured_product_id', value: featuredProductEnabled ? featuredProductId : '' },
        ], { onConflict: 'key' });
            ...
            <Dialog open={settingsDialogOpen} onOpenChange={setSettingsDialogOpen}>
              ...
              <DialogContent className="sm:max-w-md max-h-[85vh] overflow-y-auto">
                ...
                <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">رابط حساب تيك توك</label>
                    <Input
                      type="url"
                  ...
                  <div className="space-y-2">
                   ...
                   <Separator />
                   <div className="space-y-2">
                     <label className="text-sm font-medium">عدد المنتجات الحديثة المعروضة</label>
                     <Input
                       type="number"
                       min="1"
                       max="20"
                       ...
                       onChange={(e) => setNewArrivalsCount(e.target.value)}
                       dir="ltr"
                     />
                     <p className="text-xs text-muted-foreground">
                       عدد المنتجات في قسم "وصل حديثاً" بالصفحة الرئيسية (1 - 20)
                     </p>
                   </div>
                   ...
                   <div className="space-y-3 rounded-lg border border-border p-3">
                     <div className="flex items-center justify-between gap-4">
                       <div className="space-y-1">
                         <label className="text-sm font-medium">تفعيل المنتج المميز</label>
                         <p className="text-xs text-muted-foreground">
                           يظهر كنافذة منبثقة لزوار الصفحة الرئيسية
                         </p>
                       </div>
                       <Switch
                         checked={featuredProductEnabled}
                         onCheckedChange={setFeaturedProductEnabled}
                       />
                     </div>
                     {featuredProductEnabled && (
                       <div className="space-y-2">
                         <label className="text-xs font-medium">اختر المنتج المميز</label>
                         ...
                         {featuredProductId && (() => {
                           const fp = products.find(p => p.id === featuredProductId);
                           if (!fp) return null;
                           return (
                             <div className="flex gap-3 items-center p-2 rounded-md bg-secondary/30 border border-border">
                               <img src={fp.image} alt={fp.name} className="h-14 w-14 rounded object-cover" />
                               <div className="flex-1 min-w-0">
                                 <p className="text-sm font-medium truncate">{fp.name}</p>
                                 <p className="text-xs text-primary">{fp.price} ج.م</p>
                               </div>
                             </div>
                           );
                         })()}
                ...
                <Form {...form}>
                  <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>وصف قصير (يظهر في بطاقة المنتج)</FormLabel>
                          <FormControl>
                            <Input
                          ...
                          <p className="text-xs text-muted-foreground">حتى 100 حرف — اختياري</p>
                          <FormMessage />
                        </FormItem>
                      )}
                    />
                    ...
                    <div className="space-y-3 rounded-lg border border-border p-3">
                      ...
                      <div className="flex items-center justify-between">
                        <span className="text-sm">{isAvailable ? 'متوفر' : 'غير متوفر'}</span>
                        <Switch checked={isAvailable} onCheckedChange={setIsAvailable} />
                      </div>
                      {isAvailable && (
                        <div className="space-y-1">
                          <label className="text-xs text-muted-foreground">عدد القطع المتاحة (اختياري — فارغ = غير محدود)</label>
                          <Input
                            type="number"
                            min="0"
                            step="1"
                      ...
                      <p className="text-xs text-muted-foreground">
                        {!isAvailable
                          ...
                          : `متوفر — ${parseInt(stockCount, 10) || 0} قطع${
                              (parseInt(stockCount, 10) || 0) <= 5 ? ' فقط' : ''
                            }`}
                      </p>
                    </div>
                    ...
                    <div className="space-y-2 rounded-lg border border-border p-3">
                      <div className="flex items-center justify-between gap-3">
                        <div className="space-y-0.5">
                          <h4 className="font-medium text-sm">وصل حديثاً ✨</h4>
                          <p className="text-xs text-muted-foreground">
                            عرض هذا المنتج في قسم "وصل حديثاً" بالصفحة الرئيسية
                          </p>
                        </div>
